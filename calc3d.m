function z_matrix = calc3d(picFormat, c_pond, ...
    activate_smooth, ... 
    smooth_factor, folder_name,inverse_hight, ...
    ground_hight_factor, img_rotation, limiter_ponderation, ...
    limiter_area, limiter_status)
% CALC3D returns z_matrix of the different points from images
%   PicFolder needs to be called img_sequenz
%   If making strips of 1 pixel and smoothing turned on and factor 0.005
%   takes little less then 5 min
%   Linear function is used for the recontruction of missing values. The
%   spline option causes to many problems that can not all be fixed cleanly.
%
%   Example:
%     CALC3D(lot's of params)
%
...
% Load the png files in it
% Img should be saved with "img000.png" naming for a coherent sequence
...
for current_img_no = 1:no_of_img                                            % Go through all the IMGs in directory
    ...
    img_y_length = size(current_img,1);                                     % Get hight of current IMG
    no_of_strips = size(current_img,2);
    if(current_img_no == 1)                                                 % Only do this once
        z_matrix = NaN(no_of_strips,no_of_img);                      % Init matrix at NaN
    end
